...
# get team logo
logos <- gamedata |> select(name_team, url_team_season_logo)
...
teamgameshots <-pbp_group_test |> 
  ...
  select(gameteam_id, id_game, team, home, pct_3pt, pct_2pt, pct_ft, att_3pt, att_2pt, att_ft, made_ft)
...
teamgame <- teamgame |>
  group_by(team) |>
  mutate(mean_3pa = mean(att_3pt, na.rm = TRUE),
         from_mean_3pa = att_3pt - mean_3pa,
         pct_diff_3pa = (att_3pt-mean_3pa)/ mean_3pa)
